...
const PriceSearchResults = () => {
  ...
  const maxPrice = query.get('max') || 999999;
  
  // Filter states
  const [filter, setFilter] = useState({
    sort: "price_asc" // Default sort for price search is low to high
  });
  
  const fetchPriceSearchResults = async () => {
    try {
      setIsLoading(true);
      await getConfig();
      ...
      const response = await axiosInstance.get(`${GET_PRODUCT_BY_PRICE}?${params.toString()}`);
  ...
  useEffect(() => {
    fetchPriceSearchResults();
    // Reset to first page when price range changes
    setCurrentPage(1);
  }, [minPrice, maxPrice, filter.sort, currentPage]);
      ...
      {isLoading ? (
        ...
        <div className="bg-gray-50 border border-gray-200 text-gray-700 px-6 py-12 rounded-md text-center">
          ...
          <p className="text-gray-500 mb-6">
            We couldn't find any products in the price range {formatPriceRange()}
          </p>
          <button
            onClick={() => navigate('/')}
