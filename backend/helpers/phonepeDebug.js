...
const uiCheckoutTestUrl =
  "https://api-preprod.phonepe.com/apis/pg-sandbox/pg/checkout/ui/v2/pay";
const uiCheckoutProductionUrl =
  "https://api.phonepe.com/apis/pg/checkout/ui/v2/pay";
...
export const testPhonePeV2UiCheckout = async () => {
  try {
    const isProduction = process.env.PHONEPE_ENVIRONMENT === "production";
    console.log(
      "Testing PhonePe V2 UI Checkout in",
      isProduction ? "PRODUCTION" : "TEST",
      "environment"
    );
...
export const getDirectCheckoutUrl = (orderId, amount) => {
  ...
  const amountInPaise =
    typeof amount === "number" ? Math.round(amount * 100) : amount;
